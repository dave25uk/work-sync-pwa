...
// State Management
let currentViewDate = new Date(2026, 3, 1); // Default to April 2026
...
// 1. Initialize/Render Calendar Grid
async function initCalendar(date) {
    ...
    // Pull saved data for this month
    ...
    if (data) {
        data.forEach(entry => {
            ...
            if (displayEl) {
                ...
                else displayEl.classList.add('text-slate-600');
            }
        });
    }
}

// 3. Picker Logic (Manual Selection)
function openPicker(year, month, day) {
    ...
    SHIFTS.forEach(shift => {
        ...
        btn.innerText = shift;
        btn.onclick = async () => {
            // Update UI with shorthand immediately
...
document.getElementById('fetchBtn').onclick = async () => {
    ...
    try {
        ...
        // Save each fetched shift to the DB
        ...
        // Refresh grid to show new shifts
        initCalendar(currentViewDate);
        alert('iCloud Sync Complete!');
    } catch (err) {
        console.error('Sync Error:', err);
        alert('Could not sync iCloud calendar.');
    } finally {
